...
for i, (e0,e1) in enumerate(zip(erg_bins[0:-1],erg_bins[1:])):
    c1.cd(i+1)
    histSP = TH1F(24, 180, binwidths=bin_width/2)
    histDP = TH1F(24, 180, binwidths=bin_width/2)
    ...
    if i == 0:
        norm_factor = 1./max(histSP.binvalues**2)

    histSP *= norm_factor
    histSP = histSP.Rebin(2)
    title = '{0:.1f}<E<{1:.1f}'.format(e0,e1)
    histSP.SetTitle(title)

    if 2*max(histSP.binvalues)>global_max:
        global_max = max(histSP.binvalues)
