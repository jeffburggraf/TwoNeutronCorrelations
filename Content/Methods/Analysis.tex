...
The efficiency and acceptance of the neutron detection system varies greatly over its opening angle range from 20$^{\circ}$ to 180$^{\circ}$, as illustrated in Fig.~\ref{fig:DetAcceptance}.
...
In order to give a result that is sensitive to angular correlations but insensitive to detector efficiencies and experimental drifts (in PMT voltage, accelerator current, etc.), angular correlation is calculated as follows:
\begin{equation}
...
\caption{Raw two-neutron opening angle measurement from the photofission of $^{238}$U. 
This distribution is highly influenced by the detection system's geometry and efficiency.
}
\label{fig:DetAcceptance}
\end{figure}

The construction of $nn_{\text{uncorr}}(\theta)$ is achieved by examining pulses in pairs of two.
The pulses of each pulse-pair were required to have occurred within 0.2 seconds of each other so that both pulses were very likely to have occurred under similar experimental conditions.
Then, for pulse-pairs which have neutron events in both pulses, the opening angle is calculated normally between all available neutron pairs formed under the restriction that neutrons from the same pulse are not paired together.
Only pulse-pairs that had two neutron events in both pulses were used, and, as a result, four pairs of uncorrelated neutrons could be formed within each pulse-pair.
This was done in order to increase the percentage of pulse-pairs comprising fission neutrons as opposed to neutrons from $(\gamma,n)$.
This is an important point because the detection of multiple neutrons from $(\gamma,n)$ are completely removed from $nn_{\text{corr}}(\theta)$, the numerator in Eq.~\ref{eq:angularCorr}, by the subtraction of accidental coincidences, but in order to minimize the dependence of the result on detector geometry/efficiency, the numerator and denominator must comprise neutron pairs that have the same joint energy distribution.

Fig~\ref{fig:erg_distributions} shows the energy distribution of correlated neutrons (~$nn_{\text{corr}}(\theta)$~), different pulse neutrons using only pulses that had two neutron events (~$nn_{\text{uncorr}}(\theta)$~), and different pulse neutrons without a restriction on the number of events in a given pulse. 
Two binary operations, the mean (~$0.5(E_{1} + E_{2})$~) and absolute difference (~$|E_1 - E_2|$~), are shown because the relevant condition here is that the \emph{joint} energy distributions of neutron-neutron pairs from $nn_{\text{corr}}(\theta)$ and $nn_{\text{uncorr}}(\theta)$ are similar.
In other words, there are two variables at hand, namely, the energy of both neutrons.
Because these two variables are correlated in $nn_{\text{corr}}(\theta)$, it is not sufficient to consider the energies of all neutrons individually, by, for example, calculating the global average.
As seen in Fig.~\ref{fig:erg_distributions}, the neutron energy distributions of $nn_{\text{uncorr}}(\theta)$ and  $nn_{\text{corr}}(\theta)$ are quite similar.
...
The two-neutron opening angle distribution of accidental neutron coincidences from the photo-disintegration of D$_{2}$O is expected to be flat.
