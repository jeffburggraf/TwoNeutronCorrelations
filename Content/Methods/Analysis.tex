...
In order to give a result that is sensitive to angular correlations, but is not sensitive to detector efficiencies and experimental drifts in PMT voltage, accelerator current, etc. , angular correlation is determined by dividing a correlated neutron distribution by an uncorrelated neutron distribution:
\begin{equation}
...
This is the reason for using only pulse-pairs that have two events in each pulse, which increases the percentage of pulse-pairs comprising coincident neutrons from fission as opposed to accidental coincident neutrons from $(\gamma,n)$.
...
When examining differences between the neutron energy distributions of the numerator and denominator in Eq.~\ref{eq:angularCorr}, it must be considered how the energies of both neutrons in n-n pairs vary together, or, in other words, their joint energy distribution.
...
The following three sets of n-n pairs are plotted here: correlated n-n pairs (~$nn_{\text{corr}}(\theta)$~), different pulse n-n pairs using only pulses that had two neutron events (~$nn_{\text{uncorr}}(\theta)$~), and different pulse n-n pairs with no restriction on the number of events in a given pulse.
Based off the data in Fig.~\ref{fig:erg_distributions}, it is expected that the differences in the joint neutron energy distributions of n-n pairs from $nn_{\text{uncorr}}(\theta)$ and $nn_{\text{corr}}(\theta)$ do not significantly affect the calculation of angular correlation.
