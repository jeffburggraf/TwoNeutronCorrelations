...
In order to give a result that is sensitive to angular correlations but insensitive to detector efficiencies and experimental drifts (in PMT voltage, accelerator current, etc.), angular correlation is calculated by the following fraction:
\begin{displaymath}
\text{angular correlation }  \propto \frac{nn_{\text{corr}}(\theta)}{nn_{\text{uncorr}}(\theta)},
\end{displaymath}
where $nn_{\text{corr}}(\theta)$ is the accidental-coincidence subtracted opening angle distribution, the determination of which is described in section~\ref{Reconstruction of Accidental Coincidence}, and $nn_{\text{uncorr}}(\theta)$\footnote{While this notation implies that coincident events are only due to neutrons, about 3\% of total  $nn_{\text{corr}}(\theta)$ events are not due to neutrons. This was determined by comparing data from a non-neutron producing Al target to that from a $^{238}$U target (see Fig.~\ref{fig:Noise})} refers to the uncorrelated neutron distribution, which is produced by the pairing of neutron events that occurred during different pulses.
...
The construction of $nn_{\text{uncorr}}(\theta)$ is achieved by examining pulses in pairs of two, which are required to have occurred within 0.2 seconds of each other to ensure both pulses occurred under similar experimental conditions.
Next, for the pulse-pairs that have events in both pulses, the opening angle is calculated between events across the two pulses.
Only pulse-pairs that have two neutron events in both pulses are used, and, as a result, four pairs of uncorrelated neutrons can be formed within each pulse-pair.
The reason for this is to increase the percentage of fission neutrons used in $nn_{\text{uncorr}}(\theta)$, as opposed to neutrons from $(\gamma,n)$.
...
In order for the result to not depend on detector efficiency, the numerator and denominator must be comprised of neutron pairs with the same joint energy distribution.

Fig~\ref{fig:erg_distributions} shows how the energy distribution of neutron pairs varies between correlated neutrons, different pulse neutrons using all pulses, and different pulse neutrons using only pulses that had two neutron events ($nn_{\text{uncorr}}$). 
...
As seen in Fig.~\ref{fig:erg_distributions}, the neutron energy distributions of $nn_{\text{uncorr}}$ and  $nn_{\text{corr}}(\theta)$ are quite similar.
