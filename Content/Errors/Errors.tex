...
\begin{figure}[h]
    ...
    \caption{Uncertainties in opening angle determined by the propagation of position uncertainties through the opening angle calculation.
     ...
     For this reason, the mean uncertainty of measurements falling within each angle bin is plotted.
...
This is a cause for concern, because the scattering of neutrons from heavy nuclei is very likely to deflect neutrons at large angles, creating two-neutron opening angles that are not reflective of the true opening angle immediately after fission.
...
MCNP was used to simulate the production of pairs of fission neutrons uniformly throughout the target volume with energies typical for fission neutrons.
The probability that at least one neutron, out pairs of two neutrons, scatters before exiting the target was calculated from the simulation.
For the target used in this work, the simulation indicated that 6\% of the opening angles between neutron pairs once they exit the target have been altered due to scattering within the target.

A thin strip is the ideal target shape with regard to the rate of neutron elastic scattering per unit volume.
See Fig~\ref{fig:ElasticScatteringPlot} for the result of an MCNP simulation of the elastic scattering rates for both thin strip and cylindrical targets.
For this work, which used a thin strip target with a width 40 times greater than its thickness, the simulation indicated that the rate of elastic scattering is roughly a factor of two less than for a cylindrical target of the same volume.
\begin{figure}
    \centering
    ...
    \caption{
     Result of an MCNP simulation in which neutron-neutron pairs, with energies sampled from a typical watt fission spectrum, were generated uniformly throughout the volume of DU targets.
        The y-axis is the rate of opening angle contamination due to the scattering of, within the DU target in which they were produced, either one or both neutrons in neutron-neutron pairs.
    ...
    The point on the x-axis label ``this work'' corresponds to the target used in this work, which had a length of 4~cm, a width of 2~cm, and a thickness of 0.05~cm.
    }
