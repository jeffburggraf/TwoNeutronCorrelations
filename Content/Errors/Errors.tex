...
Recent works that measured the neutron-neutron angular correlations in the spontaneous fission of $^{252}$Cf and $^{240}$Pu~\cite{Pozzi2016,Verbeke2018} addressed this effect by using an MCNP-PoLiMi simulation to estimate and then subtract cross-talk from their measurements.
...
Figure~\ref{fig:CrosstalkVScoincidence} shows the distribution of cross-talk events and true neutron-neutron coincidences as a function of reconstructed opening angle.
...
\begin{figure}
    ...
    \caption{
    MCNP-PoLiMi simulation of the number of cross-talk events \emph{versus} correlated neutron-neutron events as a function of reconstructed opening angle.
...
This is a cause for concern because when neutrons scatter from heavy nucleons, such as $^{238}$U, they are likely to be deflected at large angles, resulting in neutron-neutron opening angles that do not reflect the true underlying fission kinematics.
...
The probability that at least one neutron out of a pair of two scatters before exiting the target was calculated from the simulation, indicating that 6\% of measured neutron-neutron opening angles were perturbed due to scattering.
...
%Fig~\ref{fig:ElasticScatteringEffect} compares the neutron-neutron opening angle distribution at the moment immediately after emission (denoted by \emph{true} in figure) to that of neutrons once they have escaped the target (denoted by \emph{reconstructed} in figure).
